...
export const settings = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      fields: [
        defineField({
          ...
          // `accept` only filters the picker; drag-drop bypasses it, so
          // validation is the enforcement. Refs end in `-<ext>`.
    ...
    defineField({
      ...
      description:
        "The fallback image shown when a page is shared on social media (Open Graph / Twitter). Used whenever a page has no image of its own. Recommended size 1200×630.",
