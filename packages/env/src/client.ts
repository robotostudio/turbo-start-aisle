...
const env = createEnv({
  ...
  client: {
    ...
    /**
     * Canonical origin, e.g. `https://example.com`. Required off Vercel, where
     * the `VERCEL_*` vars above are never injected. On Vercel it still wins over
     * the generated `*.vercel.app` url.
     */
